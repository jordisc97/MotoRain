...
```mermaid
%%{init: {'theme': 'dark', 'themeVariables': { 'primaryColor': '#0f172a', 'primaryTextColor': '#f1f5f9', 'lineColor': '#94a3b8', 'mainBkg': '#0f172a', 'nodeBorder': '#334155' }}}%%
flowchart LR
    classDef data     fill:#1d4ed8,stroke:#60a5fa,stroke-width:1px,color:#fff
    classDef process  fill:#b45309,stroke:#fbbf24,stroke-width:1px,color:#fff
    classDef model    fill:#059669,stroke:#34d399,stroke-width:1px,color:#fff
    classDef ensemble fill:#064e3b,stroke:#34d399,stroke-width:2px,color:#fff,font-weight:bold
    classDef explain  fill:#6d28d9,stroke:#a78bfa,stroke-width:1px,color:#fff
    classDef output   fill:#9d174d,stroke:#f472b6,stroke-width:2px,color:#fff,font-weight:bold
    classDef monitor  fill:#1e3a8a,stroke:#3b82f6,stroke-width:1px,color:#fff

    subgraph ING ["① Ingestion"]
        A1["📂 Raw Data<br/>(Customers/Usage)"]:::data
        A2["🧹 Normalise &<br/>Drift Analysis"]:::process
        A1 --> A2
    end

    subgraph FE ["② Features"]
        F1["⏱️ Rolling Windows<br/>& Industry Maps"]:::process
        F2["🎯 Target Label<br/>(90d Window)"]:::process
        F1 --> F2
    end

    subgraph BT ["③ Backtesting"]
        B1["📅 Sequential Split<br/>Feb - Jul 2020"]:::process
        B2["🔍 RFE Selection<br/>Top 30 Features"]:::process
        
        subgraph Models
            direction TB
            M1["🌲 Random Forest"]:::model
            M2["⚡ LightGBM"]:::model
            M3["📐 Logistic Reg"]:::model
        end
        
        B6["🗳️ Soft-Voting<br/>Metamodel"]:::ensemble
        
        B1 --> B2 --> Models --> B6
    end

    subgraph XP_EV ["④ & ⑤ Analysis"]
        direction TB
        XP["🔬 SHAP Explainer<br/>+ GPT Briefs"]:::explain
        EV["📊 ROC-AUC: 0.82<br/>PSI Drift Check"]:::monitor
    end

    subgraph SO ["⑥ Output"]
        E1["🏆 Top-10 Lead List<br/>(Sunday CSV)"]:::output
        E2["📡 Monitoring &<br/>Stability Plan"]:::monitor
    end
